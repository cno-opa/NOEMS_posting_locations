...
rank.polygon.impact <- function(impact.points, polygons, rank.to = 2, seed.polygons = NULL) {
	...
	ordered.set <- order_by_coverage(impact.points, initial.polygons)
	...
	if (rank.to > 1){
		for (i in 2:rank.to) {
			...
			ordered.set <- order_by_coverage(uncovered.calls, remaining.polygons )
...
remove.by.NewName <- function(polygon_NewNames, polygon.set) {
	#Removes the polygon from polygon.set via ID variable
	polygon.set[!(as.character(polygon.set$NewName) %in% c(as.character(polygon_NewNames))),]
}
# 
# 
